...
tpl.title("line segment-bezier intersection");
...

{
    tpl.addSection("separate", true);
    {
        ...
        const lineSegment = new LineSegment([0, 0], [5, -5]);
        const bezier = new Bezier([6, -5], [-2, 10], [16, 6], [-19, -6]);
        trileanResult(card, lineSegment, bezier, inter.separate(lineSegment, bezier));
    }
}
{
    ...
    {
        const card = tpl.addCard({ title: "triple lines bezier separate" });
