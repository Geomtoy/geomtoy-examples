import Geomtoy, { Bezier, QuadraticBezier, Intersection } from "@geomtoy/core";
import { Utility } from "@geomtoy/util";
import tpl from "../../assets/templates/tpl-renderer";
import { arrayResult, trileanResult } from "./_common";

tpl.title("Quadratic bezier-bezier intersection");
...
}
{
    tpl.addSection("Separate");
    {
        ...
        trileanResult(card, quadraticBezier, bezier, inter.separate(quadraticBezier, bezier));
    }
}
{
    tpl.addSection("Intersect");
    {
        ...
        const quadraticBezier = new QuadraticBezier([0, 0], [10, 20], [10, 5]);
        const bezier = new Bezier([0, 0], [10, 20], [10, 5], [-10, -10]);
        arrayResult(card, quadraticBezier, bezier, inter.intersect(quadraticBezier, bezier));
    }
    {
    ...
    {
        const card = tpl.addCard({ title: "triple lines bezier and triple lines bezier intersect" });
        const bezier1 = new Bezier([-5, 10], [5, -10], [2, -4], [0, 0]);
        const bezier2 = new Bezier([1, 2], [5, 10], [-10, -20], [0, 0]);
        arrayResult(card, bezier1, bezier2, inter.intersect(bezier1, bezier2));
    }
    {
        const card = tpl.addCard({ title: "common bezier and triple lines bezier intersect" });
        const bezier1 = new Bezier([5, -5], [5, -11], [-16, 7], [25, 13]);
        const bezier2 = new Bezier([1, 2], [5, 10], [-10, -20], [0, 0]);
        arrayResult(card, bezier1, bezier2, inter.intersect(bezier1, bezier2));
    }
