...
function treeRecursion(data: any, dirPath: string, parentElement: HTMLUListElement) {
    data.children.forEach((item: any, index: number) => {
        ...
        if (item.type === "dir") {
            ...
            const active = sessionStorage.getItem(dirPath) || "0";
            li.innerHTML = `<span class="dir ${active === "1" ? "dir-open" : ""} text-dark">${item.name}</span><ul data-path="${dirPath}" class="nested ${active === "1" ? "active" : ""}"></ul>`;
            parentElement.append(li);
