...
tpl.addMarkdown(`
Class \`Display\` is the class to which the renderer's \`display\` property correspond. 
the \`display\` can be understood as the display settings or display status.
It is responsible for setting and maintaining the transformation relationship between the view coordinate system and the screen coordinate system.
`);
