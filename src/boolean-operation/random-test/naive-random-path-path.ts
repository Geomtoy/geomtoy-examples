...
tpl.title("Random path-path test with naive strategy");
...
const path1 = new Path(Utility.range(0, 5).map(_ => randomPathCommand()));
const path2 = new Path(Utility.range(0, 5).map(_ => randomPathCommand()));
const description = bo.describe(path1, path2);

const viewCollection: View[] = [];
tpl.addMarkdown(`
Refresh to random. Each of the two paths contains 5 random commands. 
`);
